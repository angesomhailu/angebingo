...
export default function GameRoom({ params }) {
  ...
  // Simulate calling numbers every 5 seconds
  useEffect(() => {
    const interval = setInterval(() => {
      const newNumber = getRandomInt(1, 75);
      setCalledNumbers(prev => {
        if (prev.includes(newNumber) || prev.length >= 75) return prev;
    ...
    <div className="min-h-screen bg-slate-950 pt-24 pb-20 relative overflow-hidden flex flex-col">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full flex-1 flex flex-col">
        ...
        <div className="bg-slate-900/60 border border-white/10 rounded-3xl p-6 backdrop-blur-md shadow-2xl mb-8 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <button className="px-8 py-3 rounded-xl bg-gradient-to-r from-red-600 to-pink-600 text-white font-black text-lg hover:from-red-500 hover:to-pink-500 transition-all transform hover:scale-105 shadow-[0_0_20px_rgba(225,29,72,0.4)] uppercase tracking-wider">
            Bingo!
          </button>
        </div>
